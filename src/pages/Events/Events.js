					...
					<div className="dotsView">
						{elements.map((element, index) => (
							<span
								key={index}
								className="dot"
								style={{ backgroundColor: colorDots[element.chapter] }}
							></span>
						))}
					</div>
				</React.Fragment>
					...
					{sortedElements.map((element, index) => (
						...
						>
							<div className="eventsCardDate">
								<p className="eventsCardDateFont">
									{new Date(element.date).getDate()}
								</p>
								<p className="eventsCardDateFont">
									{number2month(new Date(element.date).getMonth())}
								</p>
							</div>
							<div className="eventsCardText">
								<p className="eventsCardTextFont">
									{element.description +
										' - as ' +
										new Date(element.date).getUTCHours() +
										':' +
										(new Date(element.date).getMinutes() > 10
											? new Date(element.date).getMinutes()
											: '0' + new Date(element.date).getMinutes()) +
										'h'}
								</p>
							</div>
						</div>
			...
			<div className="eventsContainer">
				<Calendar
					...
					onActiveStartDateChange={value => {
						isLoading(false);
						setDate(value.activeStartDate);
					}}
				/>
				<div
					className="eventsCardView"
					style={!loading ? { justifyContent: 'center' } : null}
				>
					{!loading ? (
						<ClockLoader size={height / 7} color={'#344ea9'} loading={true} />
					) : (
						createDatesInfo()
					)}
				</div>
